...
def dataset_split(rating_np, args):
    ...
    # user_list = set(rating_np[:, 0])
    train = '../data/' + args.dataset + '/train'
    if os.path.exists(train + '.npy'):
        train_data = np.load(train + '.npy', allow_pickle=True)
        test_data = np.load('../data/' + args.dataset + '/test.npy', allow_pickle=True)
    else:
        train_data = []
        test_data = []
        record = get_user_record(rating_np, False)
        x = 0
        for user in record.keys():
            x += 1
            print('\r', '{}/{}'.format(x, len(record)), end='')
            item_list = record[user]
            train_list = np.random.choice(list(item_list), size=int(len(item_list) * args.ratio), replace=False)
            test_list = list(item_list - set(train_list))
            for item in train_list:
                train_data.append([user, item, 1, 1])
            for item in test_list:
                test_data.append([user, item, 1, 1])
        print()
        train_data = np.array(train_data)
        test_data = np.array(test_data)
        np.save(train + '.npy', train_data)
        np.save('../data/' + args.dataset + '/test.npy', test_data)
